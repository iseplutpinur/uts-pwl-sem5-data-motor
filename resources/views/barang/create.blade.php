...
@section('content')
    <div class="container mt-4">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('barang.store') }}" method="POST" enctype="multipart/form-data" id="mainform">
                    @csrf
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" value="{{ old('kode') }}"name="kode"
                                id="kode" required placeholder="Silahkan masukan kode barang">
                        </div>
                    </div>
                    @error('kode')
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" value="{{ old('nama') }}"name="nama"
                                id="nama" required placeholder="Silahkan masukan nama barang">
                        </div>
                    </div>
                    @error('nama')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                            ...
                            <select class="form-control"name="satuan_id" id="satuan_id" required>
                                ...
                                @foreach ($satuans as $satuan)
                                    <option value="{{ $satuan->id }}"
                                        {{ $satuan->id == old('satuan_id') ? 'selected' : '' }}>
                                        {{ $satuan->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="number" class="form-control" value="{{ old('stok') }}"name="stok"
                                id="stok" required placeholder="Silahkan masukan stok barang">
                        </div>
                    </div>
                    @error('stok')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
