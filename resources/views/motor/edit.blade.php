    ...
    <div class="container mt-4">
        ...
        <div class="card">
            <div class="card-header d-md-flex flex-row justify-content-between">
                <h5 class="card-title">Ubah Motor</h5>
                <div>
            ...
            <div class="card-body">
                <form action="{{ route('motor.update', $motor->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control"value="{{ old('nama') ?? $motor->nama }}"
                                name="nama" id="nama" required placeholder="Silahkan masukan nama motor">
                        </div>
                    </div>
                    @error('nama')
                            ...
                            <select class="form-control" name="merek_id" id="merek_id" required>
                                ...
                                @foreach ($mereks as $merek)
                                    <option value="{{ $merek->id }}"
                                        {{ $merek->id == (old('merek_id') ?? $motor->merek_id) ? 'selected' : '' }}>
                                        {{ $merek->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="number" class="form-control" value="{{ old('harga') ?? $motor->harga }}"
                                name="harga" id="harga" required placeholder="Silahkan masukan harga motor">
                        </div>
                    </div>
                    @error('harga')
                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
